...
export function createApp({ manager } = {}) {
  ...
  app.post('/sessions/:managementCompanyId/messages/history', async (request, response, next) => {
    try {
      const { messageKey, timestamp, count } = request.body || {};
      response.json(await sessionManager.fetchRecentHistory(request.managementCompanyId, messageKey, timestamp, count));
    } catch (error) {
      if (error.message === '`messageKey` with `remoteJid` and `timestamp` are required.') {
        response.status(422).json({ message: error.message });
        return;
      }
